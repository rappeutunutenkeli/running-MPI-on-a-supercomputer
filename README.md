...
xychart-beta
    ...
    y-axis "Время (секунды)" 0 --> 0.15
    line "4×32" [0.11, 0.08, 0.05, 0.01]
```
## Вывод
С увеличением количества вычислительных ядер с 1 до 8 время умножения матриц фиксированного размера (200×200) сокращается с 0,11 до 0,01 секунды. Это демонстрирует эффективность параллельных вычислений на кластере: добавление ресурсов позволяет ускорить выполнение задачи более чем в 10 раз, что подтверждает правильность распараллеливания алгоритма с помощью MPI.
